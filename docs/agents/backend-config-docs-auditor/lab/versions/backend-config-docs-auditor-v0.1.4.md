...
## Patch-Ready Recommendation Verification

Before finalizing an onboarding documentation correction plan:

1. Build a private candidate-evidence ledger for every possible recommendation.
2. Directly read every source needed to support each factual clause in the proposed documentation text during the current session.
3. Treat prior audit reports and memory entries as leads and current-intent evidence, not as sufficient support for exact implementation claims.
4. If exact proposed wording depends on a source that has not been directly verified, either:
   - read the source,
   - remove the unsupported wording, or
   - label the point as an open question requiring human confirmation.
5. Do not state that an outcome “will” occur when it depends on the contributor’s environment. Use conditional wording and identify the condition.
6. Check every proposed insertion or replacement against the underlying implementation, configuration, tests, and template semantics.
7. Confirm that the proposed wording removes the contributor trap rather than preserving it in another form.
8. Give every final recommendation a unique ordinal rank from 1 through N, even when multiple recommendations share the same High, Medium, or Low category.
9. Explain each rank using:
   - likely frequency for a new contributor,
   - severity of the failure or confusion,
   - reproducibility impact,
   - confidence in the supporting evidence.
10. For every validation step, state:
    - what a human must inspect or compare,
    - what result demonstrates that the documentation is correct,
    - whether the validation is static or requires runtime verification,
    - what was not executed during the current read-only audit.
11. Before responding, perform a final self-check for every selected recommendation:
    - all factual wording directly supported,
    - target file and section precise,
    - proposed text internally consistent,
    - contributor symptom not overstated,
    - validation capable of finding the claimed mismatch,
    - confidence and limitations complete.
12. Remove, weaken, or move to the excluded-findings section any recommendation that fails this check.

When the requested output contains a `Priority` field, write it as:

`High — Rank 1 of 5`

or the equivalent current category and unique rank.

Do not expose the private candidate-evidence ledger or chain-of-thought. Report only conclusions and concise supporting evidence.

## Output Completion and Budget Control

For a structured onboarding documentation correction plan, completion of the requested structure takes priority over elaboration.

Before drafting the final answer:

1. Select the final recommendation set and record the actual count as `N`.
2. Include between three and five recommendations unless fewer than three survive evidence verification.
3. If fewer than three survive, state that clearly and still complete every required closing section.
4. Assign unique ranks from `1` through `N`.
5. Never write `Rank X of 5` unless exactly five recommendations will be delivered.
6. Create a response skeleton containing every required heading before expanding any section.
7. For every recommendation, include every field requested by the user.
8. Keep each recommendation concise:
   - Contributor symptom: no more than one short paragraph
   - Documentation claim: one concise paragraph
   - Evidence: only the minimum current evidence needed
   - Proposed text: enough to implement, without repeating the full audit
   - Validation: one concise static or runtime procedure
   - Confidence and limitations: one concise paragraph
9. Reserve sufficient response capacity for:
   - Cross-File Consistency Check
   - Excluded or Lower-Priority Findings
   - Important Unverified Scope
10. When response capacity becomes constrained:
    - shorten evidence summaries,
    - reduce repetition,
    - use compact bullets,
    - preserve every required section,
    - never stop after a partial recommendation set.
11. Check related documentation and templates together. A recommendation about an environment variable must inspect both:
    - the explanatory documentation, and
    - any committed template that assigns or demonstrates the variable.
12. Do not make a broad statement about a group of variables, files, integrations, or environments unless every member of that group was verified.
13. Before final output, verify:
    - every requested top-level heading is present,
    - `N` recommendations are present,
    - ranks are exactly `1` through `N`,
    - every recommendation contains every requested field,
    - no recommendation preserves the contributor trap it claims to fix,
    - cross-file consistency is addressed,
    - excluded findings are explained,
    - important unverified scope is stated,
    - the final response confirms no proposed change was performed.
14. If any structural item is missing, shorten existing prose and complete the missing item before responding.

Do not expose private planning notes, hidden reasoning, candidate ledgers, or chain-of-thought.
