...
def build(local_tests: int, offline_tests: int) -> None:
    ...
    story = [_header(styles), Spacer(1, 7), Paragraph("Score calculation: 52 − 1 (criterion 2) − 1 (criterion 4) − 1 (criterion 9) − 1 (criterion 11) = 48. This is a self-assessment, not a Canvas regrade.", styles["callout"]), Paragraph("Criteria 1–7", styles["section"]), _rubric_table(rows[:7], styles), PageBreak(), _header(styles), Spacer(1, 7), Paragraph("Criteria 8–13", styles["section"]), _rubric_table(rows[7:], styles), Spacer(1, 5), Paragraph("Current verification", styles["section"]), Paragraph(f"Full local evaluation suite: {local_tests} passed · Offline network-disabled container: {offline_tests} passed · GitHub CI 33517133584: SUCCESS · Public anonymous access: HTTP 200 · Video unchanged.", styles["body"]), Paragraph("Required human actions before resubmission", styles["section"])]
    for text in ("Submit the exact public repository link through Canvas/LaunchCode.", "Confirm the grader received and can open the full repository.", "Complete the walkthrough at or below 10 minutes with Muhammad."):
        story.append(Paragraph("• " + text, styles["bullet"]))
    story.append(Paragraph("Repository delivery is ready: implementation commit a7813c4 is public and passed governance CI. The remaining grading action is to submit the exact link and confirm receipt; the video-length correction remains separate.", styles["callout"]))
